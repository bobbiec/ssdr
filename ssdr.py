...
def img_retrieve_source_base64(flow, old_src):
    try:
        resp = requests.get(old_src)
    except requests.exceptions.MissingSchema:
        new_uri = f"{flow.request.scheme}://{flow.request.host}:{flow.request.port}/{old_src}"
...
def script_get_source_string(flow, old_src):
    try:
        resp = requests.get(old_src)
    except requests.exceptions.MissingSchema:
        new_uri = f"{flow.request.scheme}://{flow.request.host}:{flow.request.port}/{old_src}"
...
def response(flow: http.HTTPFlow) -> None:
    ...
    if html.body:
        ...
        with ThreadPoolExecutor(max_workers=num_workers) as executor:
            image_results = [
                executor.submit(functools.partial(img_retrieve_source_base64, flow), image['src'])
                for image in images
            ]
            script_results = [
                executor.submit(functools.partial(script_get_source_string, flow), script['src'])
